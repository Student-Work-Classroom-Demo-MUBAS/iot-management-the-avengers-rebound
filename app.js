...
// trial
(async () => {
  await syncDatabase(true); // Pass true to force sync
})();
...
// inserting logs
app.post('/api/sensors/data/bulk', async (req, res) => {
    try {
        const sensorData = req.body; // Assuming body-parser is used
        // Insert into database logic here
        await SensorData.create(sensorData.readings); // Adjust based on your model
...
// app.post('/webhook/sensor-data', async (req, res) => {
//   try {
//     console.log('Incoming ESP32 webhook data:', req.body);

//     const { deviceId, location, timestamp, readings } = req.body;

//     if (!deviceId || !readings || !Array.isArray(readings)) {
//       return res.status(400).json({ success: false, error: 'Missing required fields: deviceId or readings' });
//     }

//     const timestampDate = timestamp && !isNaN(timestamp) ? new Date(Number(timestamp)) : new Date();

//     const createdRecords = [];
//     for (const r of readings) {
//       if (!r.type || r.value === undefined || !r.unit) continue;
...
//     const existingIo = app.get('io');
//     existingIo.emit('sensor-update', createdRecords);
...
//     res.status(200).json({ success: true, message: 'All sensor data stored', data: createdRecords });
//   } catch (error) {
...
app.use((err, req, res, next) => {
    const errorCode = err.status || 500; // Default to 500 if no status is set
